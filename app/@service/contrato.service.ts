...
export class ContratoService {
  ...
  constructor(
    ...
    private httpClient: HttpClient
      ...
      this.administracionTopic.Topic = 'contrato';
  }

  public obtenerListaContrato(arrayParams:any){
    return this.http.get(URI_ADMIN_LISTA + 'contrato/', this.header.getOptions(arrayParams))
      .map(this.extractData)
      .catch(this.handleError);
  }

  public obtenerContratoPorId(id: string ): Observable <Contrato> {
    return this.http
      .get(URI_ADMIN_LISTA + 'contrato/' + id, this.header.getOptions([]))
      .map(this.extractData)
      .catch(this.handleError);
  }

  public crearContrato(contrato: Contrato) {
    this.administracionTopic.Objeto = contrato;
    return this.http.post(URI_ADMIN_CREAR, this.administracionTopic, this.header.getOptions([]))
      .catch(this.handleError);
  }

  public editarContrato(contrato: Contrato) {
    this.administracionTopic.Objeto = contrato;
